		...
		methods: {
			...
			getDeliveryAddressList() {
				...
				https.request('/rest/member/deliveryAddress/list', {
					...
					if (result.success) {
						...
						if (result.data.records.length > 0) {
							var shopAddress = result.data.records[0];
							var location = shopAddress.longitude + "," + shopAddress.latitude;
							_this.confirmChoiceShopList(location).then(result => {
								if (result.data.records.length == 0) {
									toastService.showModal('', '该区域外送功能即将开通',
										function() {});
								};
								if (result.data.records.length > 0) {
									var initShopInfo = result.data.records[0];

									app.globalData.deliveryAndSelfTaking.radioIndex = 1;
			...
			appraiseTap(e) {
				...
				let appraiseJson = {
					...
					appraiseIndex: this.appraiseIndex,
				...
				this.setData({
					...
					content: appraiseId == appraiseJson.appraiseId ? this.content : '',
					appraiseFocus: true
				});
			...
			getAppraiseTextareaHeight() {
				...
				console.log(this.appraiseHeight);
			},

			handleTap(e, _dataset) {
				/* ---处理dataset begin--- */
			...
			handleCurrencyTap(e, _dataset) {
				...
				if (e.detail.type == 'delete') {
					...
					if (e.currentTarget.dataset.type == '1') {
						url = '/rest/member/appraise/delete';
					}
					if (e.currentTarget.dataset.type == '2') {
						url = '/rest/member/reply/delete';
					}
			...
			deleteAppraise(e, url) {
				...
				https.request(url, {
					...
					if (result.success) {
						if (type == '1') {
							this.appraiseList[appraiseIndex].giveLikeCount -= 1;
							this.appraiseList[appraiseIndex].isGiveLike = true;
						}
						this.setData({
			...
			giveLikeCountTap(e) {
				...
				https.request('/rest/member/giveLike/insert', data).then((result) => {
					toastService.hideLoading();
					if (result.success) {
						if (type == '1') {
							this.appraiseList[appraiseIndex].giveLikeCount += 1;
			...
			deleteLikeCountTap(e) {
				...
				https.request('/rest/member/giveLike/delete', data).then((result) => {
					if (result.success) {
						toastService.hideLoading();
						if (type == '1') {
							this.appraiseList[appraiseIndex].giveLikeCount -= 1;
							this.appraiseList[appraiseIndex].isGiveLike = false;
						}
						this.getReplyByAppraiseId();
						// this.setData({
						//   appraiseList: this.data.appraiseList
			...
			contactBussinessTap() {
				uni.makePhoneCall({
					phoneNumber: this.shopInfo.shop.contactPhone
				});
			},
			...
			editAddress() {
				...
				uni.navigateTo({
					url: '../../address/replace/replace?jump_page=menu_index&addressType=true&shopId=' + this
						.shopInfo.shop.id
				});
			},
