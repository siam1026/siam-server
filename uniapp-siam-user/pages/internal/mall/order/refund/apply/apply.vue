<template>
    <view>
        ...
        <view class="refund-reason">
            <view class="refund-reason-top">
                ...
                <view class="refund-reason-top-right">
                    <view @tap="isAllowApplyRefund">
                        ...
                        <text v-if="notSelected" class="theme-color">{{ cancelOrderNoReasonList[oldCancelOrderNoReason].name }}</text>
                    </view>
            ...
            <view class="textarea-uploader">
                <textarea
                    ...
                    @input="bindRefundReasonDescription"
                ...
                <view class="uploader-images-view">
                    ...
                    <text class="iconfont iconshangchuantupian1" @tap="uploadImage" v-if="uploaderImages.length <= 2"></text>
                </view>
            </view>
        </view>
        ...
        <van-action-sheet :show="choiceReasonApplyDialog" title="选择取消原因" @close="close">
                ...
                <scroll-view style="height: 55vh" scroll-y>
                    <radio-group class="choiceReason-radio-group" @change="radioChange">
                        <label
                            ...
                            v-for="(item, index) in cancelOrderNoReasonList"
                            :key="index"
                        >
                            {{ item.name }}
        ...
        <van-action-sheet :show="customerServiceWechatDialog" title="客服二维码" @close="close">
            <view class="content_box">
                <scroll-view style="height: 35vh" scroll-y>
                    <view class="wechat-code-view">
                        <image
                            :src="current.customerServiceWechatQrcode"
                            :show-menu-by-longpress="true"
                            mode="aspectFit"
                            class="customerServiceWechatQrcode"
                            @tap="viewQrCodeImage"
                        ></image>
    ...
    methods: {
        ...
        checkboxChange(e) {
            ...
            for (var i in checkeds) {
                ...
                packingCharges = utilHelper.toFixed(packingCharges + items[checkeds[i]].packingCharges * items[checkeds[i]].number, 2);
                //计算后的总额加上当前商品的价格
            ...
            this.orderDetailList.forEach((item, index) => {
                ...
                if (item.checked) {
                    ...
                    if (this.order.fullReductionRuleId) {
                        ...
                        let sharePrice = utilHelper.toFixed(price * limitedPrice_, 2);
                        console.log(item.goodsName + '==x' + item.number + '=========>' + sharePrice);
                        fullActualPrice = utilHelper.toFixed(fullActualPrice + utilHelper.toFixed(sharePrice, 2), 2);
                    }
