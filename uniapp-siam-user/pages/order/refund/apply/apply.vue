<template>
	<view>
		<view class="top-detail">
			...
			<view class="top-shop-name">
				<view>
					<checkbox :checked="isSelectAll" @tap="bindSelectAll" iconColor="#FFFFFF" style="width: 100%;">全选
					</checkbox>
				...
				<view class="refund-reason-top-right">
					<view @tap="isAllowApplyRefund">
						...
						<text v-if="notSelected"
							class="theme-color">{{ refundReasonApply[oldCancelOrderNoReason].name }}</text>
					</view>
		...
		<van-action-sheet :show="choiceReasonApplyDialog" title="选择取消原因" @close="close" @cancel="close">
				...
				<scroll-view style="height: 55vh" scroll-y>
					<radio-group class="choiceReason-radio-group" @change="radioChange">
						<label
							...
							v-for="(item, index) in refundReasonApply" :key="index">
							{{ item.name }}
...
<script>
	...
	export default {
		...
		methods: {
			...
			close() {
				this.choiceReasonApplyDialog = false;
			},
			...
			checkboxChange(e) {
				...
				for (var i in checkeds) {
					...
					packingCharges = utilHelper.toFixed(packingCharges + items[checkeds[i]].packingCharges * items[checkeds[i]]
						.number, 2);
					//计算后的总额加上当前商品的价格
